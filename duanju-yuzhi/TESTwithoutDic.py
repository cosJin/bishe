...
a = f.read()
sentences = a.split("\n\n")
f2 = open('cleaned_train0-400.txt', 'r', encoding='utf-16', errors="replace")
a2 = f2.read()
sentences2 = a2.split("\n\n")
...
vocab_size = 6153  # 样本中不同字的个数+1(padding 0)，根据处理数据的时候得到
...
def taken(sentence):
    ...
    return s
# print(sentences[0])
# print(cut_word(sentences[0]))
RigthNum = 0
ALLNum = 0

for sentenceIndex in range(len(sentences)):
    try:
        sentence_rig = sentences2[sentenceIndex]
        pre0 = cut_word(sentences[sentenceIndex])
    except:
        continue
    # print(result)
    sentence_pre = ''
    for each in pre0:
        sentence_pre = sentence_pre + each + ' '
    print(sentence_pre)
    print(sentence_rig)
    ...
    if len(pre) == len(rig):
        ...
        print(RigthNum, ALLNum)
        print(RigthNum / ALLNum)
        print("==========")



# sentence = "兒 母子 應 長 (腸)  斷 ，應 須 會 �� 見 如來 ，今日 講 經 功德 分 ，因 逢 便 早 歸來 。"#那  知州  聽   得   這  話  ，從  頂門  上  不  見  了  三魂  ，腳  底下  疏失  了  七魄  ，便  投  後 殿  走  了  。
...
# print(rss)
#
# sentence = "失卻龍駒怎戰爭，了虞姬那痛增。"#失卻  龍駒  怎  戰爭  ，了  虞姬  那  痛  增  。
# result = cut_word(sentence)
# rss = ''
# for each in result:
#     rss = rss + each + ' / '
...
#     rss = rss + each + ' / '
# print(rss)
# sentence = "今朝希遇大乘經，見優曇花一種；"#今朝  希  遇  大乘經  ，見  優曇花  一  種  ；
# result = cut_word(sentence)
# rss = ''
# for each in result:
#     rss = rss + each + ' / '
# print(rss)
# sentence = "全不見鴻門會那氣性，今日向烏江岸滅盡形。"#全  不  見  鴻門會  那  氣性  ，今日  向  烏江  岸  滅   盡   形  。
# result = cut_word(sentence)
# rss = ''
# for each in result:
#     rss = rss + each + ' / '
# print(rss)
# sentence = "昔者齊晏子使於梁國為使"#昔  者  齊  晏子  使  於  梁國  為  使
# result = cut_word(sentence)
# rss = ''
# for each in result:
#     rss = rss + each + ' / '
# print(rss)
# sentence = "劍雖三尺，定四方，麒麟雖小，聖君瑞應；"#劍  雖  三  尺  ，定  四方  ，麒麟  雖  小  ，聖君  瑞應  ；
# result = cut_word(sentence)
# rss = ''
# for each in result:
#     rss = rss + each + ' / '
